...
describe("DELETE /api/orders/:id — Delete order endpoint", () => {
  ...
  it("should return 401 when admin is not logged in", async () => {
    const res = await request(app).delete(`${route}/${objectID}`);
  ...
  it("should return 404 when order is not found by the provided id", async () => {
    ...
    const res = await agent.delete(`${route}/${objectID}`);
  ...
  it("should delete order and return 200", async () => {
    ...
    const res = await agent.delete(`${route}/${order?._id}`);
